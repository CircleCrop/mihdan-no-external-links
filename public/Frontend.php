	...
	public function parser($matches): string
	{
		...
		if ('' !== $this->options->inclusion_list) {
			...
			if (!preg_match('/^[a-z]+:\/\/[^\/]+/i', $href)) {
				$this->debug_info('Invalid link format (' . $href . '), masking...');
				return $matches[0];
			}

			$ipPattern = '/^(?:\[(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\]|(?:\d{1,3}\.){3}\d{1,3})|(?:10\.\d{1,3}\.\d{1,3}\.\d{1,3})|(?:127\.\d{1,3}\.\d{1,3}\.\d{1,3})|(?:169\.254\.\d{1,3}\.\d{1,3})|(?:192\.168\.\d{1,3}\.\d{1,3})|(?:172\.(?:1[6-9]|2\d|3[0-1])\.\d{1,3}\.\d{1,3})$/';
			if (preg_match($ipPattern, $href)) {
				$this->debug_info('Link is an internal IP address (' . $href . '), not masking...');
				return $matches[0];
			}

			// 排除列表检查
			foreach ($this->exclusion_list as $exclusion_url) {
				if (stripos($href, $exclusion_url) !== false) {
